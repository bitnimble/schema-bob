...
describe('serialization', () => {
  describe('bool', () => {
    ...
    it('fails to serialize a bool of the wrong literal type', () => {
      ...
      // @ts-expect-error
      expect(() => literalTrue.serialize(false)).toThrow(
        'Expected literalTrue to be true but found type boolean instead, with value false',
      );
    });
  });
  ...
  describe('str', () => {
    ...
    it('fails to serialize a string of the wrong literal type', () => {
      ...
      // @ts-expect-error
      expect(() => literalHelloWorld.serialize('invalid')).toThrow(
        'Expected literalHelloWorld to be one of values ["hello", "world"] but found "invalid" instead',
      );
    });
  });
  ...
  describe('num', () => {
    ...
    it('fails to serialize a number of the wrong literal type', () => {
      ...
      // @ts-expect-error
      expect(() => literal1234.serialize(5)).toThrow(
        'Expected literal1234 to be one of values [1, 2, 3, 4] but found 5 instead',
      );
    });
  ...
  describe('optional', () => {
    ...
    it('allows undefined values for record properties', () => {
      ...
      const val: Reify<typeof someRec> = { prop1: undefined, prop2: undefined };
  ...
  describe('rec', () => {
    it('can round-trip a shallow record type', () => {
      ...
      const val1: Reify<typeof someRec> = { prop1: true, prop2: 'hello world', _prop3: undefined };
    ...
    it('can round-trip a nested record type', () => {
      const someRec = rec('someRec', {
        nestedRec: rec('nestedRec', { prop1: str('nestedProp1'), prop2: str('nestedProp2') }),
        prop1: str('topLevelProp1'),
        prop2: bool('topLevelProp1'),
      });
      const val: Reify<typeof someRec> = {
        nestedRec: { prop1: 'hello', prop2: 'world' },
        prop1: 'test',
        prop2: true,
      };
      expect(rt(someRec, val)).toEqual(val);
    ...
    it('avoids serializing properties not present in the schema', () => {
      ...
      const someSubRec = rec('someSubRec', { prop: str('prop') });
      const val = { prop: 'hello world', prop2: 'this should disappear', prop3: 'begone demons' };
      expect(rt(someSubRec, val)).toEqual({ prop: 'hello world' });
      expect(() => someRec.deserialize(someSubRec.serialize(val))).toThrow(
        'Expected prop2 to be string but found type undefined instead, with value undefined',
      );
    });
  ...
  describe('extend', () => {
    it('can round-trip an extended record', () => {
      const extended = extend(
        'extended',
        rec('base', { prop1: str('prop1'), prop2: bool('prop2') }),
        { prop3: str('prop3'), prop4: optional(bool('prop4', true)) },
      );
    ...
    it('can override properties on the base type', () => {
      ...
      const val: Reify<typeof extended> = { prop1: 'hello', prop2: 'world' };
      expect(rt(extended, val)).toEqual(val);
    });
  });
    ...
    it('throws an error if the subschema does not include the discriminator property', () => {
      expect(() => {
        union('someUnion', 'success', [
          rec('someRec1', { success: bool('success', true) }),
          // @ts-expect-error
          rec('someRec2', { prop1: bool('prop1') }),
        ]);
      })
        .toThrow(
          'Subschema "someRec2" of union type "someUnion" is missing discriminator property "success"',
        );
    });

    it('can discriminate on string literal properties', () => {
      const someUnion = union('someUnion', 'kind', [
        rec('someRec1', { kind: str('kind', 'hello'), foo: bool('foo') }),
        rec('someRec2', { kind: str('kind', 'goodbye'), bar: bool('bar') }),
        rec('someRec2', { kind: str('kind', 'greetings'), baz: bool('baz') }),
      ]);
      const val: Reify<typeof someUnion> = { kind: 'greetings', baz: true };
  ...
  describe('list', () => {
    ...
    it('can round-trip a list of nested records', () => {
      ...
      const val: Reify<typeof someList> = [{
        prop1: 'hello',
        nested: { nestedProp1: true, nestedProp2: 'beep' },
      }, { prop1: 'world', nested: { nestedProp1: false, nestedProp2: 'boop' } }];
      expect(rt(someList, val)).toEqual(val);
    });

    it('can round-trip a list of lists', () => {
      const someList = list('someList', list('nestedList', num('nestedListItem')));
      const val = [[1, 4, 6], [494, 182, 344], [-23, -67, 42]];
      expect(rt(someList, val)).toEqual(val);
    });
  });
});
